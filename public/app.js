...
ngApp.controller("mypins", function ($scope, $compile, $http) {
  $scope.$on("$stateChangeSuccess", function () {
    ...
    if (typeof $http !== "function") {
      console.error(
        "%cExpecting $http to be type of function.  $http currently is " +
          typeof $http,
        "background-color:black; color: red; font-size:18px"
      );
    }
    ...
    $http(htConfig).then(
      function (response) {
        ...
        for (var i = 0; i < response.data.length; i++) {
          ...
          if (
            $("#li-sign-out").data("username") ==
            response.data[i].pinOwner[0].userName
          ) {
          ...
          pinInfo.setAttribute(
            "ui-sref",
            `user({username: '${response.data[i].pinOwner[0].userName}'})`
          );
...
ngApp.controller("newPin", function ($scope, $http) {
  ...
  var updatePreview = function () {
    ...
    if ($("#image-placeholder")[0].children.length > 0) {
      if (
        $("#image-placeholder")[0].children[0].tagName == "IMG" &&
        $("#addNewForm input[name=url]")[0].value ==
          $("#image-placeholder")[0].children[0].tagName.src
      ) {
...
ngApp.controller("recent", function ($scope, $location, $compile, $http) {
  ...
  $scope.$on("$locationChangeSuccess", function () {
    ...
    console.log(
      "Location change success!  $location.path is " +
        $location.path() +
        " while document.location.hash is " +
  ...
  $scope.$on("$stateChangeSuccess", function () {
    ...
    console.log(
      "State change success!  $location.path is " +
        $location.path() +
        " while document.location.hash is " +
        document.location.hash
    );
    ...
    $http(htConfig).then(
      function (response) {
        ...
        if (response.data.status == "vote added") {
          ...
          document.querySelector(
            '[data-pin-id="' + pin2star + '"] .vote-button .star-count'
          ).innerText =
            parseInt(
              document.querySelector(
                '[data-pin-id="' + pin2star + '"] .vote-button .star-count'
          ...
          document
            .querySelector('[data-pin-id="' + pin2star + '"] .vote-button')
            .classList.add("voted-button");
        }
      ...
      function (response) {
        ...
        if (response.status == 401) {
          //If we get unauthorized, we need to sign in.  This takes care of that.
  ...
  $scope.delete = function (evntVar) {
    ...
    $http({
      ...
      if (
        ...
        window.alert(
          "There was an error while trying to remove Pin: " +
            response.data.error
        );
      }
...
$(function () {
  ...
  $(".navbar-nav>li>a").click(function (event) {
    if (event.clientX == 0 && event.clientY == 0) {
      //Keyboard was used instead of mouse (as clientX & Y is the mouse's position)
      //console.dir(event);
      $(event.target.parentElement).addClass("navKbSelect");
      $(event.target.parentElement).one(
